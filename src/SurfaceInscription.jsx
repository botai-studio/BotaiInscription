  ...
  useEffect(() => {
    ...
    mesh.traverse((child) => {
      if (child.isMesh && child.geometry) {
        ...
        for (let t = 0; t < numTris; t++) {
          ...
          // ONLY include triangles facing the same direction as the click normal
          // This prevents dots from appearing on the back side of the mesh
          const dotProduct = faceNormal.dot(basis.normal);
          if (dotProduct < 0.1) continue; // Skip back-facing triangles
    ...
    for (const dot of uvDots) {
      ...
      if (bestTriangle && bestBary && 
        ...
        // Offset slightly along face normal (always use click normal direction)
        surfacePos.add(basis.normal.clone().multiplyScalar(0.005));
        worldDots.push({
          position: surfacePos,
...
export { ClickMarker, SurfaceRaycaster, SurfaceText, UVMapVisualization };
